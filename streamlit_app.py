...
with st.sidebar:
    # --- MODIFICATION 1: Make Data Source Collapsible ---
    with st.expander("⚙️ Data Source & Settings", expanded=False): # Collapsed by default
        ...
        if use_google:
            if SHEET_ID_SECRET:
                ...
                SHEET_ID = st.text_input("Google Sheet ID", value="")

            if RANGE_SECRET: RANGE = RANGE_SECRET
            else: RANGE = st.text_input("History sheet name", value="History Transactions")

            if APPEND_RANGE_SECRET: APPEND_RANGE = APPEND_RANGE_SECRET
            else: APPEND_RANGE = st.text_input("Append sheet name", value="Append Transactions")

            if CREDS_FILE_SECRET: CREDS_FILE = CREDS_FILE_SECRET
            else: CREDS_FILE = st.text_input("Service Account JSON (optional)", value="creds/service_account.json")
        else:
    ...
    st.markdown("---") # Separator moved outside the expander
...
def _read_sheet_with_index(spreadsheet_id: str, range_name: str, source_name: str, creds_info, creds_file):
    ...
    if not df.empty: df['_sheet_row_idx'] = df.index.astype(int)
...
def sample_data():
    ...
    for i in range(30):
        ...
        t = "credit" if i % 7 == 0 else "debit"; amt = abs(amt) # Ensure amount is positive for sample
...
if data_source == "Upload CSV/XLSX":
    uploaded = st.file_uploader("Upload CSV or XLSX", type=["csv", "xlsx"]) # Now outside sidebar
...
with st.spinner("Cleaning data..."):
    ...
    # Ensure Amount is numeric early
    amt_col_raw = next((c for c in converted_df.columns if c.lower() == 'amount'), None)
    if amt_col_raw:
        converted_df[amt_col_raw] = pd.to_numeric(converted_df[amt_col_raw], errors='coerce')
    # Ensure Type column exists and is lowercase
    type_col_raw = next((c for c in converted_df.columns if c.lower() == 'type'), None)
    if type_col_raw:
         converted_df['type_normalized'] = converted_df[type_col_raw].astype(str).str.lower().str.strip()
    else:
         # Basic heuristic if no type column (adjust if needed)
         if amt_col_raw:
             converted_df['type_normalized'] = converted_df[amt_col_raw].apply(lambda x: 'credit' if pd.notna(x) and x < 0 else 'debit')
             converted_df[amt_col_raw] = converted_df[amt_col_raw].abs() # Make amounts positive after inferring type
         else:
             converted_df['type_normalized'] = 'unknown'


# --- Define Filters in Sidebar Expanders ---
sel_banks = []
min_amount_filter = 0.0
sel_types = ['debit', 'credit'] # Default
start_sel, end_sel = None, None # Will be set below
...
with st.sidebar:
    ...
    with st.expander("📊 Chart & Metric Options", expanded=False):
        ...
        try: # Use overall totals for stable year/month options
        ...
        except: all_years = [datetime.utcnow().year]; merged_all_totals = pd.DataFrame()
        ...
        metric_month_choices = list(month_map_chart.values())
        ...
        st.caption("Uses IQR. Affects only top-right metric.")

    # --- Expander 2: Transaction Filters ---
    with st.expander("🔍 Transaction Filters", expanded=True):
        st.write("**Filter Transactions By**")
        banks_available = sorted([b for b in converted_df['Bank'].unique() if pd.notna(b) and b != 'Unknown']) + ['Unknown']
        ...
        min_amount_filter = st.number_input(
            "Amount >= (0 to disable)", min_value=0.0, value=0.0, step=100.0, format="%.2f", key="min_amount_filter"
        )

        # --- MODIFICATION 2: Add Type Filter Widget ---
        transaction_types = ['debit', 'credit']
        sel_types = st.multiselect(
            "Type(s)",
            options=transaction_types,
            default=transaction_types, # Default to both
            key="sel_types"
        )
        # --- END MODIFICATION 2 ---
        ...
        except: max_date_overall = datetime.utcnow().date(); min_date_overall = max_date_overall - timedelta(days=365)

        totals_mode = st.radio("Mode", ["Single date", "Date range"], index=0, key="totals_mode")

        if totals_mode == "Single date":
            today = datetime.utcnow().date(); default_date = max(min_date_overall, min(today, max_date_overall))
            ...
            s_raw, e_raw = dr if isinstance(dr, (tuple, list)) and len(dr) == 2 else (dr, dr)
...
# --- Apply Core Filters (Bank, Amount, Type) ---
converted_df_filtered = converted_df.copy()

if sel_banks: # Apply Bank Filter
    converted_df_filtered = converted_df_filtered[converted_df_filtered['Bank'].isin(sel_banks)]

if min_amount_filter > 0.0: # Apply Amount Filter
    amt_col = next((c for c in converted_df_filtered.columns if c.lower() == 'amount'), None)
    if amt_col:
        try: converted_df_filtered = converted_df_filtered[converted_df_filtered[amt_col] >= min_amount_filter].copy()
        except: pass # Ignore if amount column wasn't numeric
    else: st.warning("Amount filter needs 'Amount' column.")

# --- MODIFICATION 3: Apply Type Filter ---
if sel_types and 'type_normalized' in converted_df_filtered.columns:
    # Ensure sel_types contains only valid lowercase types before filtering
    valid_sel_types = [t.lower().strip() for t in sel_types if isinstance(t, str)]
    if valid_sel_types: # Apply filter only if valid types are selected
         converted_df_filtered = converted_df_filtered[converted_df_filtered['type_normalized'].isin(valid_sel_types)].copy()
# --- END MODIFICATION 3 ---


# --- Compute daily totals (Uses globally filtered data) ---
...
# --- Top-Right Metric (Unchanged logic, uses pre-filter totals) ---
...
def _month_year_to_date(y_str, m_name): try: y=int(y_str) except: y=datetime.utcnow().year; try: m=pd.to_datetime(m_name, format='%B').month except: m=datetime.utcnow().month; return y,m
def compute_month_avg_from_merged(mrg_df, yr, mo, replace_outliers=False): #... (logic unchanged) ...
...
with col_c: # Metric display (logic unchanged)
...
# --- Rows view & download (Uses globally filtered data + date range) ---
...
_desired = ['timestamp', 'bank', 'type', 'amount', 'message'] # Removed 'suspicious' as it wasn't used
...
if rows_df.empty:
     st.info("No rows match all selected filters for this date range.")
     display_df = pd.DataFrame(columns=['Timestamp', 'Bank', 'Type', 'Amount', 'Message']) # Empty structure for download
elif not display_cols: # Fallback
    st.warning("Could not find preferred columns."); display_df = rows_df
else:
    ...
    # Amount is already numeric from global filter step
    # if amt_col: display_df[amt_col] = pd.to_numeric(display_df[amt_col], errors='coerce') # Ensure again?
    type_col_disp = next((c for c in display_df.columns if c.lower() == 'type'), None) # Find original 'Type' if exists
    if type_col_disp: display_df = display_df.rename(columns={type_col_disp: 'Type'}) # Use original casing if possible
    elif 'type_normalized' in display_df.columns: display_df = display_df.rename(columns={'type_normalized': 'Type'}) # Use normalized as fallback
...
# --- Build selectable mapping for Delete UI (Uses fully filtered rows_df) ---
selectable = False; selectable_labels = []; selectable_label_to_target = {}
if use_google and io_mod and not sheet_full_df.empty:
    map_df = rows_df.copy() # Use the final filtered rows_df
...
if use_google and io_mod and not sheet_full_df.empty:
    ...
    with col_b: remove_btn = st.button("Remove selected", key="remove_rows_btn", disabled=not selectable or not selected_labels) # Disable if nothing selected
    if not selectable and not use_google: st.info("Connect to Google Sheet to enable row removal.")
    elif not selectable: st.info("Row selection unavailable (cannot map rows to sheet).")
...
# --- Add New Row UI (Unchanged logic) ---
if use_google and io_mod:
    ...
    with st.expander("Open add row form"):
        with st.form("add_row_form", clear_on_submit=True):
             ...
             amount = st.number_input("Amount (₹)", value=0.0, step=1.0, format="%.2f") # Use %.2f for currency
...
# --- Totals Display (Uses globally filtered + date filtered data) ---
totals_heading = f"Totals — {start_sel}" if start_sel == end_sel else f"Totals — {start_sel} → {end_sel}"
st.markdown(f"### {totals_heading}")
try:
    sel_df = rows_df.copy() # Use the final rows_df (filtered by bank, amount, type, date)
    ...
    if sel_df.empty: st.info(f"No transactions match all filters for this date range.")
        ...
        # Amount is already numeric and positive (if type was inferred)
        if type_col_norm:
        ...
        else: st.warning("Cannot calculate totals accurately: 'Type' column missing or could not be inferred.")
