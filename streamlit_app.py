# streamlit_app.py - FIXED VERSION (Copy-Paste Ready)
...
def calculate_running_balance(transactions_df: pd.DataFrame, balance_df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['Signed_Amount'] = df.apply(
        lambda r: r['Amount'] if r['Type_norm'] == 'credit' else -r['Amount'], axis=1
    )
    ...
    # Sort and merge
    df = df.sort_values(by=['timestamp', '_sheet_row_idx']).merge(bal_info, on='Bank', how='left')
    ...
    final_df = transactions_df.merge(
        df[['_temp_uid', 'Balance']], on='_temp_uid', how='left'
    ).drop(columns=['_temp_uid'])
...
if use_google:
    ...
    with st.spinner("Fetching Google Sheets..."):
        ...
        if history_df.empty and append_df.empty:
            st.error(f"No data found in Google Sheet '{SHEET_ID}' ranges '{RANGE}' or '{APPEND_RANGE}'.")
            st.stop()
...
# ------------------ Transform and calculate balances ------------------
...
with st.sidebar:
    ...
    with st.expander("🔍 Transaction Filters", expanded=True):
        ...
        sel_types = st.multiselect("Transaction Type(s)", options=type_options, default=type_options, key="sel_types")
...
if min_amount_filter > 0.0:
    ...
    if amount_col_name:
        ...
        st.warning("Amount filter needs 'Amount' column.")
...
def compute_month_avg_from_merged(mrg_df, yr, mo, replace_outliers=False):
    ...
    if len(vals) < 3 or not replace_outliers: 
        return _safe_mean(vals), len(vals), {"n": len(vals)}
...
if plot_df.empty:
    ...
    if charts_mod is not None:
        ...
        if show_debit_chart: 
            series_selected_chart.append('Total_Spent')
        if show_credit_chart: 
            series_selected_chart.append('Total_Credit')
        try:
...
# FIXED: Table display with Balance column (capital B)
_desired = ['timestamp', 'bank', 'type', 'amount', 'Balance', 'message']  # FIXED: Capital B
if 'subtype' in [c.lower() for c in converted_df_with_balance.columns]:
...
if ts_col: 
    display_df[ts_col] = pd.to_datetime(display_df[ts_col], errors='coerce').dt.strftime('%Y-%m-%d %H:%M').fillna('')

amt_col = next((c for c in display_df.columns if c.lower() == 'amount'), None)
if amt_col: 
    display_df[amt_col] = pd.to_numeric(display_df[amt_col], errors='coerce')

bal_col = next((c for c in display_df.columns if c.lower() == 'balance'), None)
if bal_col: 
    display_df[bal_col] = pd.to_numeric(display_df[bal_col], errors='coerce')
...
if use_google and io_mod is not None and not sheet_full_df.empty:
    ...
    if '_sheet_row_idx' in map_df.columns and '_source_sheet' in map_df.columns:
        for i, r in map_df.iterrows():
            ...
            selectable_label_to_target[label] = (tgt_range, idx)
    if selectable_labels: 
        selectable = True

if use_google and io_mod is not None and not sheet_full_df.empty:
    st.markdown("---")
    st.write("🗑️ Bulk Actions (Google Sheet only)")
    ...
    with col_a:
        ...
        if not selectable: 
            st.info("Row selection unavailable (cannot map rows to sheet).")
    with col_b: 
        remove_btn = st.button("Remove selected", key="remove_rows_btn", disabled=not selectable)
    ...
    if remove_btn and selected_labels:
        ...
        for lbl in selected_labels:
            ...
            rng, idx = tgt if tgt else (None, None)
            if rng and idx is not None: 
        ...
        for rng, indices in groups.items():
            try:
                ...
                any_error = True
        if not any_error: 
            st.success(f"Marked {total_updated} rows deleted.")
            st.cache_data.clear()
            st.experimental_rerun()
...
try:
    ...
    if sel_df.empty:
        ...
        if type_col:
            ...
            credit_sum = sel_df.loc[credit_mask, amount_col].sum()
            debit_sum = sel_df.loc[debit_mask, amount_col].sum()
