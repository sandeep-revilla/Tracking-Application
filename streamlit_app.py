...
import hashlib  # CHANGED: used for stable widget keys
...
# CHANGED: compute unread_count defensively from raw notif_df (before we filter out read items)
unread_count = 0
...
if st.session_state.get('show_notif_panel', False):
    ...
    if notif_df.empty:
        ...
        # Track which notification is expanded (store hashed uid in session state for stability)
        ...
        for _, nrow in display_notif.iterrows():
            ...
            n_created   = str(nrow.get('created_at', '') or '')
            n_is_read   = str(nrow.get('is_read', 'false')).lower() == 'true'

            # CHANGED: build a stable hashed key for widgets (avoids DuplicateWidgetID)
            safe_uid = hashlib.md5(f"{n_uid}|{n_created}".encode()).hexdigest()
            toggle_key = f"toggle_{safe_uid}"
            verify_key = f"verify_inline_{safe_uid}"

            is_expanded = st.session_state.get('_expanded_notif_uid') == safe_uid
            ...
            with row_right:
                # Toggle button — expand or collapse (CHANGED: stable key)
                if st.button(arrow, key=toggle_key, help="Click to expand / collapse"):
                    if is_expanded:
                        st.session_state['_expanded_notif_uid'] = None
                    else:
                        st.session_state['_expanded_notif_uid'] = safe_uid
            ...
            if is_expanded:
                ...
                if not n_is_read:
                    ...
                    with btn_col:
                        # CHANGED: stable key for verify button
                        if st.button(
                            "✅ Mark as Verified",
                            key=verify_key,
                            type="primary",
...
# (Remaining UI code for charts, rows, add/delete, totals, etc., remains unchanged
# from your original file; omitted here for brevity as they are unchanged and long.)
# If you'd like, I can paste the remaining sections as well — but nothing else needed
# for the notification & DuplicateWidgetID fixes.
