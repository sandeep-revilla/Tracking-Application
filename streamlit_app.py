...
def _cached_notif_df(sheet_id, _creds_info, creds_file):
    """Cache notification reads for 60s to avoid hammering the API."""
...
# Sync ALL transactions (not just filtered) into Notifications sheet on every load
if use_google and notif_mod and SHEET_ID:
    with st.spinner("Syncing notifications…"):
        notif_mod.sync_large_transactions(
            converted_df_with_balance,   # full unfiltered dataset
...
if not notif_df.empty and 'is_seen' in notif_df.columns:
    try:
        unseen_count = int((notif_df['is_seen'].astype(str).str.lower() == 'false').sum())
    except Exception:
        unseen_count = 0
...
with bell_col:
    bell_label = f"🔔 {unseen_count}" if unseen_count > 0 else "🔔"
    if st.button(
        bell_label,
        key="bell_btn",
        help=f"{unseen_count} unseen notification(s)" if unseen_count > 0 else "No unseen notifications",
    ):
...
if st.session_state.get('show_notif_panel', False):
    ...
    with panel_title_col:
        st.markdown(
            ...
            f"border-radius:12px;font-size:14px'>{unseen_count} unseen</span>",
            unsafe_allow_html=True,
        )
    with mark_all_col:
        if unseen_count > 0:
            if st.button("✅ Mark all as seen", key="mark_all_seen_btn"):
                if use_google and notif_mod and SHEET_ID:
    ...
    # Show only unseen notifications in the panel
    if not notif_df.empty and 'is_seen' in notif_df.columns:
    ...
    if display_notif.empty:
        ...
        for _, nrow in display_notif.iterrows():
            ...
            border   = "#dee2e6" if n_is_seen else "#f5c2c7"
            dot      = "⚪" if n_is_seen else "🔴"
            ...
            with row_right:
                if st.button(arrow, key=f"toggle_{n_uid}", help="Expand / collapse"):
                    if is_expanded:
                        st.session_state['_expanded_notif_uid'] = None
                    else:
                        st.session_state['_expanded_notif_uid'] = n_uid
                        # Mark as seen when opened ← key change
                        if not n_is_seen:
                            if use_google and notif_mod and SHEET_ID:
                                notif_mod.mark_notification_seen(
                                    SHEET_ID, n_uid,
            ...
            if is_expanded:
                ...
                status_color = "#6c757d" if n_is_seen else "#dc3545"
                status_label = "✅ Seen" if n_is_seen else "🔴 Unseen"
...
if plot_df.empty and chart_type_select not in ("Weekly heatmap", "Debit vs Credit pie", "Bank breakdown", "Day-of-week pattern"):
...
if can_delete:
    ...
    if st.session_state.get("_show_delete_popup") and selected_row_indices:
        ...
        preview_cols = [c for c in ['Timestamp', 'Bank', 'Type', 'Amount', 'Message'] if c in display_df.columns]
        popup.dataframe(
