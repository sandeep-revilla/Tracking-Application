...
# ------------------ UPDATED: Balance Calculation (Only debit + account/bank_transfer) ------------------
def calculate_running_balance(transactions_df: pd.DataFrame, balance_df: pd.DataFrame) -> pd.DataFrame:
    ...
    Only updates balance when Type=debit AND Subtype=account/bank_transfer.
    Ignores: credits, card/PayZapp/wallet transactions, transactions before Start_Date.
    """
    ...
    # Ensure timestamp and normalize Type
    ...
    # Create signed amount (credits +, debits -)
    ...
    if bal_info['Start_Date'].isnull().any():
        st.warning("Some banks missing Start_Date. Using earliest date.")
    ...
    # UPDATED LOGIC: Only debit + account/bank_transfer affects balance
    def get_effective_amount(row):
        # Ignore if before start date
        if row['timestamp'].date() < row['Start_Date'].date():
            return 0.0
        
        # Ignore card AND PayZapp/wallet transactions
        if row['subtype_norm'] in ['card', 'payzapp', 'payzapp wallet', 'wallet']:
            return 0.0
        
        # ONLY update balance for: Type=debit AND Subtype=account/bank_transfer
        is_debit_account = (
            row['Type_norm'] == 'debit' and 
            row['subtype_norm'] in ['account', 'bank_transfer', 'bank transfer']
        )
        
        return row['Signed_Amount'] if is_debit_account else 0.0
...
with st.spinner("Cleaning data and calculating balances..."):
    ...
    # Calculate running balance (UPDATED LOGIC)
...
if use_google and io_mod is not None:
    ...
    with st.expander("Open add row form"):
        with st.form("add_row_form", clear_on_submit=True):
             ...
             subtype = st.selectbox("Subtype", options=["bank_transfer", "card", "upi", "payzapp", "wallet", "other"], index=0)
