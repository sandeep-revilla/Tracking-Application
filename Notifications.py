...
def _write_all_notifications(spreadsheet_id: str, df: pd.DataFrame, svc) -> bool:
    ...
    try:
        ...
        # Step 2: Write header + all data rows fresh
...
# ─────────────────────────────────────────────────────────────
# Hard delete ALL notification rows (keeps header)
# ─────────────────────────────────────────────────────────────
...
def delete_all_notifications(
    ...
    Uses values().clear() to reliably wipe all content, then restores
    the header row. This is the only reliable way to fully empty a sheet.
    Returns True on success.
    """
    try:
        svc = _get_write_service(creds_info, creds_file)
        ...
        # Step 1: Check how many rows currently exist
        res    = svc.spreadsheets().values().get(
            spreadsheetId=spreadsheet_id,
            range=NOTIF_SHEET
        ).execute()
        ...
        # Step 2: Clear ALL content in the sheet reliably
        svc.spreadsheets().values().clear(
            spreadsheetId=spreadsheet_id,
            range=NOTIF_SHEET,
            body={}
        ...
        # Step 3: Write header back so sheet structure is intact
        svc.spreadsheets().values().update(
            spreadsheetId=spreadsheet_id,
            range=f"{NOTIF_SHEET}!A1",
            valueInputOption="USER_ENTERED",
        ...
        print(f"[notifications] Hard deleted {row_count - 1} notification row(s). Header restored.")
        return True
...
# ─────────────────────────────────────────────────────────────
# Reload all notifications from scratch based on threshold
# ─────────────────────────────────────────────────────────────
